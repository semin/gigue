  ...
  class CLI
    ...
    def self.execute(stdout, arguments=[])
      ...
      globalopts  = OptionParser.new do |opts|
        opts.banner = <<-BANNER
GIGUE (ver #{VERSION}): A Sequence-Structure Homology Detection Method Using Environment-Specific Substitution Tables

Usage: #{File.basename($0)} [--version|-v] [--help|-h] COMMAND [ARGS]
...
GIGUE commands are:
    ...
    align     align query sequence(s) or alignment with target profile(s)
        ...
        opts.on("-v", "--version", "display program version information") {
