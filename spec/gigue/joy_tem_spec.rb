...
describe JoyTem do
  ...
  describe "when input is a path for file," do
  ...
  describe "when input is a string for file contents," do

    before(:all) do
      @file_str   = IO.read(File.join(File.dirname(__FILE__), '..', 'd.240.1.1.tem'))
      @joytem = JoyTem.new(@file_str)
    end

    it "#file returns source file location" do
      @joytem.file.should == @file_str
    end
