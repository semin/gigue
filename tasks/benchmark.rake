  ...
  task :recog_dna do
    ...
    Bio::FlatFile.open(Bio::FastaFormat, scop40) do |fasta_file|
      fasta_file.each do |entry|
        if sid_to_sccs[entry.entry_id.gsub(/^g/, 'd')].match(/^[a|b|c|d|e|f|g]/)
          seqs << Sequence.new(entry.aaseq, entry.entry_id.gsub(/^g/, 'd'), "sequence;#{entry.definition}")
        else
    ...
    fm.manage do
      dna_tems.each_with_index do |dna_tem, i|
        ...
        fm.fork do
          [ dna_esst64_sigma0_01, dna_esst128_sigma0_01, ].each do |esst|
            env   = esst.to_s.match(/(esst\d+)/)[1]
            stp   = StructuralProfile.new(dna_tem, esst)
            hits  = []
            ...
            sorted_hits = hits.sort_by { |h| h[-1] }.reverse
